...
impl ConversationView {
    ...
    fn refresh_draft_session_before_first_prompt(
        &mut self,
        expected_session_id: acp::SessionId,
        connection: Rc<dyn AgentConnection>,
        work_dirs: PathList,
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            let install_result = cx.update(|window, cx| {
                this.update(cx, |this, cx| -> Result<_> {
                    ...
                    let current_thread = current_thread.read(cx);
                    if current_thread.session_id() != &expected_session_id
                        || !current_thread.is_draft_thread()
                    {
