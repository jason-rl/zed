...
### Cookie fallback {#youtube-cookie-fallback}

YouTube sometimes asks yt-dlp to sign in to confirm you are not a bot. By default,
Zed does not access browser cookies or cookie files. To enable a fallback for this
specific error, add one of these options to your user settings:
...
The `browser` value is passed as one argument to yt-dlp's `--cookies-from-browser`.
It accepts yt-dlp's `BROWSER[+KEYRING][:PROFILE][::CONTAINER]` syntax, for example
`"chrome:Profile 1"`. Your operating system may prompt for access to the browser's
cookie store. Alternatively, use an existing Netscape-format cookies file:
...
```json [settings]
{
  "background_media": {
    "youtube_cookie_fallback": {
      "type": "cookies",
      "path": "~/private/youtube-cookies.txt"
    }
  }
}
```

The file path must be absolute or start with `~/`. yt-dlp reads the file and may
update it. Protect it like a password; do not paste cookie contents into settings
or commit them to a repository. See the
[yt-dlp cookie guidance](https://github.com/yt-dlp/yt-dlp/wiki/FAQ#how-do-i-pass-cookies-to-yt-dlp).

Each player session starts without cookies. Only the bot-verification error
activates the configured fallback, which remains active for later metadata,
artwork, and download requests in that session. This includes livestream
reconnections and resolution changes. A failed authenticated attempt is reported
normally, without another immediate authentication retry. Other failures do not
activate cookies, and cached replays still make no YouTube requests. If a download
has already written partial video data, Zed restarts that download with cookies
after stopping its decoder, rather than appending a second video to the partial
file. This recovery can briefly interrupt playback.

Set `"youtube_cookie_fallback": null` to disable it. Settings changes restart the
player session. Cookie options are passed only to yt-dlp; authentication is not
guaranteed to resolve every YouTube restriction.
